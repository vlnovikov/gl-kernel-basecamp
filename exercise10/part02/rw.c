...
static void reverse_interval(char *a, char *b, int make_upper)
{
	size_t l = (b - a) / 2;
	size_t i;
	for (i = 0; i <= l; ++i, ++a, --b) {
...
static void reverse_string(char *str)
{
	...
	char *se = str + strlen(str) - 1;
	char *ws, *we;
	reverse_interval(str, str + len - 1, 1);
	we = strchr(str, ' ');
	if (!we) {
		return;
	}
	for (ws = str; we; ws = we) {
		we = strchr(ws, ' ');
		reverse_interval(ws, we ? we - 1 : se - 1, 0);
		if (we) {
			++we;
		}
	}
